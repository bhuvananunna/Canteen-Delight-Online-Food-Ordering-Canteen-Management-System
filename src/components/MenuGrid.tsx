      ...
      {items.map((item, index) => {
        ...
        return (
          <motion.div
            ...
            className="group bg-[#121212] rounded-3xl border border-stone-800/80 hover:border-stone-700 shadow-lg hover:shadow-black/40 transition-all duration-300 overflow-hidden flex flex-col justify-between"
          >
            {/* Image Section */}
            <div className="relative h-44 w-full bg-stone-900 overflow-hidden">
              ...
              {/* Diet Tag Overlay */}
              <div className="absolute top-3 left-3 bg-[#0a0a0a]/90 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center space-x-1 shadow-sm">
              ...
              {/* Availability tag */}
              {!item.isAvailable && (
                <div className="absolute inset-0 bg-black/70 backdrop-blur-[1px] flex items-center justify-center text-center">
                  <span className="bg-red-600 text-white font-sans text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full shadow-md">
                    Out of Stock
                  </span>
                </div>
              )}
              ...
              {/* Rating overlay */}
              {item.isAvailable && (
                <div className="absolute bottom-3 right-3 bg-[#0a0a0a]/80 backdrop-blur-xs px-2 py-1 rounded-lg flex items-center space-x-1 shadow-md text-white border border-stone-850">
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between">
                  <h4 className="text-base font-bold text-stone-200 group-hover:text-amber-500 transition-colors tracking-tight font-sans line-clamp-1">
                    {item.name}
                  </h4>
              ...
              <div className="mt-4 pt-4 border-t border-stone-850 flex items-center justify-between">
                ...
                {item.isAvailable && (
                  <button
                    ...
                    className="bg-stone-900 hover:bg-stone-800 text-amber-500 px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer shadow-xs border border-stone-800"
                  >
                    {hasOptions ? (
                      <>
                        <span>Customize</span>
