...
export const NotificationToast: React.FC = () => {
  ...
  const selected = toast ? (config[toast.type] || config.info) : config.info;
  const ToastIcon = selected.icon;

  return (
    <AnimatePresence>
      {toast && (
        <div key="toast-wrapper" className="fixed bottom-5 right-5 z-50 pointer-events-none max-w-sm w-full font-sans">
          <motion.div
            key={`toast-item-${toast.message}`}
